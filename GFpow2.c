...
void GFpow_mul(const GFpow *a,const GFpow *b, GFpow *c) {
    ...
    if((a->val == 0) || (b->val == 0)) {
        c->val = 0;
        return;
    } else if(j=lg[a->val] + lg[b->val], j >= pow){
        j -= powm1;
    }
    c->val = ex[j];
...
void GFpow_mul_int(const GFpow *a, int b, GFpow *c) {
    GFpow b1 = GFpow_init_with_value(b);
    GFpow_mul(a, &b1, c);
...
void GFpow_div(const GFpow *a,const GFpow *b, GFpow *c) {
    ...
    if(b->val == 0){
        ...
        c->val = 0;
    } else if(j=lg[a->val] - lg[b->val], j < 0){
        j += powm1;
        c->val = ex[j];
    }
}
void GFpow_int_div(int a,const GFpow *b, GFpow *c) {
    GFpow a1 = GFpow_init_with_value(a);
    GFpow_div(&a1, b, c);
}
